...
NUM_SIMS     = 2000            # total sims
hosts = [h for h, ok in results if ok]
chosen_hosts = hosts[0:20]
...
def launch_sim(host, sim_id):
    ...
    try:
        ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
        ssh.connect(host, username=USER, timeout=5)
        ssh.exec_command(f"mkdir -p /home/u6068690/logs12x8/{sim_id}/{host}")
        cmd = (
            f"source /home/u6068690/.tcshrc; "
            f"python3 /home/u6068690/Scripts/12x8Scripts/remoteRunAndScore12x8.py --id {sim_id}"
        )
...
for i in range(num_batches):
    ...
    for sim_id in current_batch_ids:
        result_file = os.path.join("/home/u6068690/12x8ResearchProject/Arrangements/", f"sim_{sim_id}/", f"loss_{sim_id}")
        try:
